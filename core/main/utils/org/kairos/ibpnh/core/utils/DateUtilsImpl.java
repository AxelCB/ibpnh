...
import org.kairos.ibpnh.core.json.E_DateFormat;
...
public class DateUtilsImpl implements I_DateUtils {
	 ...
	 */
	private DateFormat dateTimeFormatISO8601 = null;
	
	/**
	 * Date time format ISO 8601.
	 */
	private DateFormat dateFormatWithoutDaysShort = null;
	
	/**
	 * Date time format with Hyphens Without Milliseconds.
	 */
	private DateFormat dateTimeFormatWithHyphensWithoutMilliseconds = null;
	...
	private final static Long MINUTES_IN_HOUR = 60L;
	private final static Long HOURS_IN_DAY = 24L;
	private final static Long MILLIS_IN_DAY = MILLIS_IN_SECOND
	...
	public void init() {
		...
		try {
			...
			Map<String, String> formatsMap = CollectionUtils
					.collectionToMap(
							formats,
							new CollectionUtils.CollectionToMapConverter<String, String, ParameterVo>() {
			...
			this.setDateTimeFormatISO8601(new SimpleDateFormat(formatsMap
					.get(ParameterVo.DATETIME_FORMAT_ISO8601), locale));
	 ...
	 * (non-Javadoc)
	 * 
	 * @see I_DateUtils#refreshFormats()
	 */
	@Override
	 ...
	 * (non-Javadoc)
	 * 
	 * @see I_DateUtils#format(java.util.Date,
	 * E_DateFormat)
	 */
	@Override
	public String format(Date date, E_DateFormat dateFormat) {
		...
		case DATE_TIME_ISO6801_FORMAT:
			return this.formateDateTimeISO8601(date);
		case DATE_FORMAT_WITHOUT_DAYS_SHORT:
			return this.formateDateWithoutDaysShort(date);
		case DATE_TIME_WITH_HYPHENS_WITHOUT_MILLISECONDS:
			return this.formateDateTimeWithHyphensWithoutMilliseconds(date);
		}
	...
	public Date parseDateTimeISO8601(String f) {
		Date d = new Date();
		try {
			d = this.getDateTimeFormatISO8601().parse(f);
		} catch (ParseException e) {
		}
	...
	public Date parseDateWithoutDaysShort(String f) {
		Date d = new Date();
		try {
			d = this.getDateFormatWithoutDaysShort().parse(f);
		} catch (ParseException e) {
		}
	...
	public Date parseDateTimeWithHyphensWithoutMilliseconds(String f) {
		Date d = new Date();
		try {
			d = this.getDateTimeFormatWithHyphensWithoutMilliseconds().parse(f);
		} catch (ParseException e) {
		}
	...
	public String formateDateTimeISO8601(Date f) {
		...
		return this.getDateTimeFormatISO8601().format(f);
	}
	...
	public String formateDateWithoutDaysShort(Date f) {
		...
		return this.getDateFormatWithoutDaysShort().format(f);
	}
	...
	public String formateDateTimeWithHyphensWithoutMilliseconds(Date f) {
		...
		return this.getDateTimeFormatWithHyphensWithoutMilliseconds().format(f);
	}
	 ...
	 * (non-Javadoc)
	 * 
	 * @see I_DateUtils#daysBetween(java.util.Date,
	 * java.util.Date, java.lang.Boolean)
	 */
	@Override
	public Long daysBetween(Date from, Date to, Boolean includeEnd) {
		Date enteredDate = this.zeroHour(from);
		Date leaveDate = this.zeroHour(to);
		Long millisDiff = leaveDate.getTime() - enteredDate.getTime();

		return (millisDiff / MILLIS_IN_DAY)
				+ ((includeEnd != null && includeEnd) ? 1 : 0);
	}
	...
	/**
	 * @return the dateTimeFormat8601
	 */
	private DateFormat getDateTimeFormatISO8601() {
		return dateTimeFormatISO8601;
	}

	/**
	 * @param dateTimeFormat8601 the dateTimeFormat8601 to set
	 */
	private void setDateTimeFormatISO8601(DateFormat dateTimeFormat8601) {
		this.dateTimeFormatISO8601 = dateTimeFormat8601;
	}

	/**
	 * @return the dateFormatWithoutDays
	 */
	private DateFormat getDateFormatWithoutDaysShort() {
		return dateFormatWithoutDaysShort;
	}

	/**
	 * @param dateFormatWithoutDaysShort the dateFormatWithoutDays to set
	 */
	private void setDateFormatWithoutDaysShort(DateFormat dateFormatWithoutDaysShort) {
		this.dateFormatWithoutDaysShort = dateFormatWithoutDaysShort;
	}

	/**
	 * @return the dateTimeFormatWithHyphensWithoutMilliseconds
	 */
	private DateFormat getDateTimeFormatWithHyphensWithoutMilliseconds() {
		return dateTimeFormatWithHyphensWithoutMilliseconds;
	}

	/**
	 * @param dateTimeFormatWithHyphensWithoutMilliseconds the dateTimeFormatWithHyphensWithoutMilliseconds to set
	 */
	private void setDateTimeFormatWithHyphensWithoutMilliseconds(
			DateFormat dateTimeFormatWithHyphensWithoutMilliseconds) {
		this.dateTimeFormatWithHyphensWithoutMilliseconds = dateTimeFormatWithHyphensWithoutMilliseconds;
	}
